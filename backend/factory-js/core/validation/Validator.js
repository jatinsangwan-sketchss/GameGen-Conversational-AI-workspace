...
export async function validateProject({
  ...
  // Read-only inspection operations should be validated using their returned
  // scene/node data, not via runtime debug-history fetching.
  // `inspectDebugOutput()` calls the executor's `get_debug_output` tool, which
  // we avoid here when the validation focuses exclusively on scene inspection.
  const debugOps = collectExecutedOperations({ explicitOperations: executedOperations, projectState });
  const shouldRunRuntimeDebugValidation = hasRuntimeDebugValidationOps(debugOps);
  const debugExcerpt = shouldRunRuntimeDebugValidation ? inspectDebugOutput({ executor, warnings }) : "";
...
function hasRuntimeDebugValidationOps(executedOperations) {
  const ops = Array.isArray(executedOperations) ? executedOperations : [];
  if (ops.length === 0) return false;
  const runtimeDebugOps = new Set(["run_project", "get_debug_output", "analyze_project"]);
  return ops
    .map((op) => normalizeExecutedOperation(op)?.action)
    .filter(Boolean)
    .some((a) => runtimeDebugOps.has(String(a)));
}
...
function buildExpectationChecks({ root, operation, expectations, runtimeResult }) {
  ...
  // Read-only scene inspection must be validated using returned scene/node
  // data (not runtime debug-history).
  const extraction = {
    execution: isPlainObject(result?.output?.execution) ? result.output.execution : null,
  };
  const execOutput = isPlainObject(extraction.execution?.output) ? extraction.execution.output : null;
  const nodes =
    execOutput?.nodes ??
    result?.output?.nodes ??
    result?.output?.execution?.output?.nodes ??
    result?.nodes ??
    result?.scene_nodes ??
    null;
  const properties =
  ...
  if (action === "list_scene_nodes") {
    const ok = Array.isArray(nodes) && nodes.length > 0;
    checks.push({
      expectation: "scene_nodes_returned",
      pass: ok,
      validation_type: "read_only_scene_validation",
      evidence: { nodes: Array.isArray(nodes) ? nodes.length : null },
      reason: ok ? null : "Expected non-empty nodes array from list_scene_nodes.",
    });
    return checks;
  }

  if (action === "get_node_properties") {
    // Read-only node property inspection is valid even when the returned
    // property map is empty. Only require object presence/shape.
    const ok = isPlainObject(properties);
    const isEmpty = ok ? Object.keys(properties).length === 0 : false;
    checks.push({
      expectation: "node_properties_returned",
      pass: ok,
      validation_type: "read_only_scene_validation",
      evidence: {
        has_properties: ok,
        empty_properties: isEmpty,
        keys: isPlainObject(properties) ? Object.keys(properties) : [],
      },
      reason: ok
        ? (isEmpty ? "Empty property set returned." : null)
        : "Expected properties object from get_node_properties.",
    });
    return checks;
  }

  if (action === "set_node_properties") {
    const semanticCheck = result?.output?.semantic_check;
    const saveSceneResult = result?.output?.save_scene_result;
    const semanticMatch = Boolean(semanticCheck && semanticCheck.match === true);
    const saveOk = saveSceneResult == null ? false : Boolean(saveSceneResult?.ok === true);
    checks.push({
      expectation: "set_node_properties_verified",
      pass: semanticMatch,
      validation_type: "inspect_first_mutation_validation",
      evidence: {
        semantic_match: semanticMatch,
        per_key: Array.isArray(semanticCheck?.per_key) ? semanticCheck.per_key : [],
      },
      reason: semanticMatch ? null : "Expected post-mutation property verification to match requested values.",
    });
    checks.push({
      expectation: "set_node_properties_saved",
      pass: saveOk,
      validation_type: "inspect_first_mutation_validation",
      evidence: { save_ok: saveOk },
      reason: saveOk ? null : "Expected save_scene to succeed after set_node_properties.",
    });
    return checks;
  }
  ...
  if (action === "add_node") {
    ...
    const saveOk = saveSceneResult == null ? false : Boolean(saveSceneResult?.ok === true);
    checks.push({
      expectation: "add_node_verified_with_parent",
      pass: semanticMatch,
      validation_type: "inspect_first_mutation_validation",
      evidence: {
        semantic_match: semanticMatch,
        expected: semanticCheck?.expectation ?? null,
        found: semanticCheck?.actual?.nodes_found ?? [],
      },
      reason: semanticMatch ? null : "Expected node name/type under resolved parent path was not found.",
    });
    checks.push({
      expectation: "add_node_saved",
      pass: saveOk,
      validation_type: "inspect_first_mutation_validation",
      evidence: { save_ok: saveOk },
      reason: saveOk ? null : "Expected save_scene to succeed after add_node.",
    });
    return checks;
  }

  if (action === "inspect_scene") {
    const okNodes = Array.isArray(nodes);
    const wantsNodeProps =
      Boolean(String(params?.node_path ?? "").trim()) ||
      Boolean(String(params?.node_name ?? "").trim()) ||
      Boolean(String(params?.target ?? "").trim()) ||
      Boolean(String(params?.target_intent ?? "").trim());
    const okProps = wantsNodeProps ? isPlainObject(properties) : true;
    const hasStructuredScene = isPlainObject(result?.output) || isPlainObject(execOutput);
    const ok = Boolean(okNodes && okProps && hasStructuredScene);
    checks.push({
      expectation: "scene_inspection_returned",
      pass: ok,
      validation_type: "read_only_scene_validation",
      evidence: { nodes_count: Array.isArray(nodes) ? nodes.length : null, properties_present: isPlainObject(properties) },
      reason: ok ? null : "Expected structured scene data (nodes/properties) from inspect_scene.",
    });
    return checks;
  }
  ...
  if (action === "get_debug_output" || expectations.requires_non_null_response === true) {
    ...
    checks.push({
      ...
      validation_type: "runtime_debug_validation",
