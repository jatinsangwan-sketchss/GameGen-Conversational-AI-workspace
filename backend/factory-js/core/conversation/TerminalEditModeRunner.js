...
function formatValueInline(value, maxLen = 200) {
...
function printReadOnlyResultSummary(readOnlyResults) {
  const items = Array.isArray(readOnlyResults) ? readOnlyResults : [];
  if (items.length === 0) return;
  for (const item of items) {
    const action = String(item?.action ?? "");
    if (!action) continue;
    if (action === "list_scene_nodes") {
      const scenePath = item?.scene_path ?? "unknown";
      const nodeCount = Number.isFinite(item?.node_count) ? item.node_count : 0;
      const truncated = item?.truncated === true;
      console.log(`Read-only result: list_scene_nodes (${scenePath})`);
      console.log(`  Node count: ${nodeCount}${truncated ? " (truncated)" : ""}`);
      const nodes = Array.isArray(item?.nodes) ? item.nodes : [];
      for (const n of nodes) {
        console.log(
          `  - ${n?.name ?? "?"} | path=${n?.path ?? "?"} | type=${n?.type ?? "?"}`
        );
      }
      continue;
    }
    if (action === "get_node_properties") {
      ...
      console.log(`Read-only result: get_node_properties (${scenePath})`);
      if (keys.length === 0) {
        console.log("  Properties: {}");
      } else {
        for (const k of keys) {
          console.log(`  - ${k}: ${formatValueInline(properties[k])}`);
        }
      }
      continue;
    }
    if (action === "inspect_scene") {
      ...
      const nodeCount = Number.isFinite(item?.node_count) ? item.node_count : 0;
      const truncated = item?.truncated === true;
      console.log(`Read-only result: inspect_scene (${scenePath})`);
      console.log(`  Node count: ${nodeCount}${truncated ? " (truncated)" : ""}`);
      const nodes = Array.isArray(item?.nodes) ? item.nodes : [];
      for (const n of nodes) {
        console.log(
      ...
      if (item?.properties && typeof item.properties === "object") {
        ...
        console.log(`  Properties keys: ${keys.length > 0 ? keys.join(", ") : "(none)"}`);
      }
    }
  }
}
...
export async function runTerminalEditMode({
  ...
  while (true) {
    ...
    // - mutation edits remain compact
    // - successful read-only edits print concise returned data for usability
    printReadOnlyResultSummary(orchestrationRes.execution?.read_only_results);
  }
}
