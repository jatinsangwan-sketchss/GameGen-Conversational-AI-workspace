...
import { SUPPORTED_RECIPES } from "../RecipeEngine.js";
...
/**
 * Intent guardrail:
 * - "list/show/inspect nodes in <scene>" are read-only *scene structure* queries.
 * - They must not be routed to `get_debug_output` (runtime/debug-history operation).
 *
 * We keep this heuristic intentionally narrow to avoid interfering with genuine
 * "debug output / logs / history" requests.
 */
function isSceneNodeListingRequest(text) {
  const t = safeString(text).trim().toLowerCase();
  if (!t) return false;
  // If the user explicitly asks for debug output/logs, do not treat as listing nodes.
  const isExplicitDebug = /\b(debug output|debug|log|logs|history|last_n|trace)\b/i.test(t);
  const hasSceneContext = /\b(scene|tscn|node tree|scene structure|nodes in)\b/i.test(t);
  if (isExplicitDebug && !hasSceneContext) return false;

  const nodeListingPatterns = [
    /\b(list|show)\b.*\b(nodes?|node)\b/i,
    /\b(nodes?|node)\b.*\b(in|for)\b.*\b(scene|tscn)\b/i,
    /\b(inspect)\b.*\b(nodes?|node)\b/i,
    /\b(scene structure)\b/i,
    /\b(what nodes)\b/i,
    /\b(node tree)\b/i,
  ];

  return nodeListingPatterns.some((re) => re.test(t)) && (hasSceneContext || /\b\.tscn\b/i.test(t));
}

function extractScenePathCandidateFromText(text) {
  const t = safeString(text);
  if (!t) return null;

  // Prefer explicit `res://...` paths.
  const resMatch = t.match(/res:\/\/[^\s"'`]+\.tscn/i);
  if (resMatch && resMatch[0]) return toResScenePath(resMatch[0]);

  // Then try explicit paths that include directories (avoid guessing bare filenames).
  const pathMatch = t.match(/([A-Za-z0-9_./-]+\/[A-Za-z0-9_./-]+\.tscn)\b/i) ?? t.match(/([A-Za-z0-9_./-]+\.tscn)\b/i);
  if (pathMatch && pathMatch[1]) {
    const candidate = String(pathMatch[1]);
    if (candidate.includes("/")) return toResScenePath(candidate);
  }

  return null;
}

function extractScenePathCandidateFromPlan(plan) {
  const candidates = [];
  const fileChanges = Array.isArray(plan?.file_changes) ? plan.file_changes : [];
  for (const fc of fileChanges) {
    const p = safeString(fc?.path).trim();
    if (p && p.toLowerCase().endsWith(".tscn")) candidates.push(toResScenePath(p));
  }
  const affected = Array.isArray(plan?.affected_project_files) ? plan.affected_project_files : [];
  for (const a of affected) {
    const p = safeString(a).trim();
    if (p && p.toLowerCase().endsWith(".tscn")) candidates.push(toResScenePath(p));
  }
  return candidates.length > 0 ? candidates[0] : null;
}

function overrideSceneNodeListingPlanOps({ plan, userRequestText }) {
  if (!isSceneNodeListingRequest(userRequestText)) return plan;
  if (!isPlainObject(plan)) return plan;

  const t = safeString(userRequestText).trim().toLowerCase();
  const wantsNodeProperties = /\b(get|show|inspect)\b.*\b(node)\b.*\b(prop|properties|attribute|attributes)\b/i.test(t) || /\bprop|properties|attribute|attributes\b/i.test(t);
  const wantsInspectScene = /\binspect\b.*\bscene\b/i.test(t) || /\bscene structure\b/i.test(t);

  const sceneFromPlan = extractScenePathCandidateFromPlan(plan);
  const sceneFromText = extractScenePathCandidateFromText(userRequestText);
  const scenePathCandidate = sceneFromText ?? sceneFromPlan;

  const ops = Array.isArray(plan?.operations) ? plan.operations : [];
  const outOps = ops.map((opObj) => {
    if (!opObj || !isPlainObject(opObj)) return opObj;
    const action = normalizeActionName(opObj.action);
    if (action !== "get_debug_output") return opObj;

    const params = isPlainObject(opObj.params) ? { ...opObj.params } : {};
    // Guardrail: node listing requests are read-only inspection, not runtime debug history.
    const nextOp = {
      ...opObj,
      action: wantsNodeProperties ? "get_node_properties" : wantsInspectScene ? "inspect_scene" : "list_scene_nodes",
      params,
    };

    const existingScene = safeString(params?.scene_path ?? params?.scenePath).trim();
    if (!existingScene && scenePathCandidate) {
      nextOp.params.scene_path = scenePathCandidate;
    } else if (existingScene) {
      nextOp.params.scene_path = toResScenePath(existingScene);
    }
    // Remove debug-specific params (like last_n) to keep output deterministic.
    delete nextOp.params.last_n;
    delete nextOp.params.lastN;
    return nextOp;
  });

  plan.operations = outOps;
  plan.mcp_actions = outOps;
  if (Array.isArray(plan.required_mcp_actions)) plan.required_mcp_actions = outOps;
  return plan;
}
...
const ACTION_LIST_FIELDS = Object.freeze([
  "affected_project_files",
  "file_changes",
  "operations",
  "mcp_actions",
...
function normalizeMissingActionListFields(plan) {
  if (!isPlainObject(plan)) return { plan, defaulted_fields: [] };
  const defaulted = [];
  ...
  if (defaulted.length > 0) {
    // LLM plans often omit empty action lists. We normalize these to preserve
    // a stable downstream shape without weakening scalar/object strictness.
    // eslint-disable-next-line no-console
    console.log("[ChangePlanner] defaulted missing action-list fields", {
      fields: defaulted,
    });
  }
  return { plan, defaulted_fields: defaulted };
}
...
function normalizeCanonicalOperationParams(action, rawParams) {
  ...
  if (op === "list_scene_nodes") {
    if (params.scene_path == null && params.scenePath != null) params.scene_path = params.scenePath;
  }
  if (op === "get_node_properties") {
    if (params.scene_path == null && params.scenePath != null) params.scene_path = params.scenePath;
    if (params.node_path == null && params.nodePath != null) params.node_path = params.nodePath;
    if (params.node_name == null && params.nodeName != null) params.node_name = params.nodeName;
    if (params.target_intent == null && params.targetIntent != null) params.target_intent = params.targetIntent;
    if (params.target == null && params.targetIntent != null) params.target = params.targetIntent;
  }
  if (op === "inspect_scene") {
    if (params.scene_path == null && params.scenePath != null) params.scene_path = params.scenePath;
    if (params.node_path == null && params.nodePath != null) params.node_path = params.nodePath;
...
function stripLegacyAliasParamsForOperation(operation, params) {
  ...
  if (operation === "get_node_properties" || operation === "inspect_scene") {
    delete out.path;
    delete out.scenePath;
    delete out.nodePath;
    delete out.nodeName;
    delete out.targetIntent;
    // Do not delete canonical snake_case `target_intent`.
    // Keep canonical `target_intent` value if the LLM used it; the
    // normalization step above already copies it into canonical keys.
    // This deletion prevents drift from mixed camel/snake keys.
  }
  return out;
}
...
function validateOperationsAgainstRegistry(plan) {
  ...
  const invalid = [];
  const kept = [];
  const recipeOpSet = new Set(Array.isArray(SUPPORTED_RECIPES) ? SUPPORTED_RECIPES : []);
  for (const op of ops) {
    const action = normalizeActionName(op?.action);
    const params = isPlainObject(op?.params) ? op.params : {};
    if (!operationExists(action)) {
      // Some prototype operations are local-only and are not part of the
      // GoPeak registry contract. Allow them through without registry
      // validation.
      if (recipeOpSet.has(action)) {
...
function constrainSupportedPrototypeOperations(supportedOperations) {
  const protoSet = new Set(Array.isArray(SUPPORTED_RECIPES) ? SUPPORTED_RECIPES : []);
  const discoveredMap = new Map(
    Array.isArray(supportedOperations)
      ? supportedOperations
          .filter((o) => o && typeof o === "object" && typeof o.operation === "string")
          .map((o) => [o.operation, o?.enabled === true])
      : []
  );

  // Ensure the planner can emit inspect_scene and create_script_file even if
  // GoPeak discovery doesn't list them (those are local prototype behaviors).
  return Array.from(protoSet).map((op) => ({
    operation: op,
    enabled:
      op === "inspect_scene" || op === "list_scene_nodes" || op === "get_node_properties" || op === "create_script_file"
        ? true
        : discoveredMap.get(op) === true,
  }));
}
...
export async function planChange({
  ...
  // Edit mode is constrained: intersect planner capabilities with the prototype
  // RecipeEngine operation set so it can't plan raw/unhandled MCP actions.
  const constrainedSupportedOperations = constrainSupportedPrototypeOperations(supportedOperations);
  ...
  const prompt = buildPromptFromTemplate(CHANGE_PLAN_PROMPT_TEMPLATE, {
    ...
    supported_operations_json: JSON.stringify(Array.isArray(constrainedSupportedOperations) ? constrainedSupportedOperations : [], null, 2),
  });
  ...
  normalizeMissingActionListFields(parsed);
  ...
  // Guardrail: scene node listing requests should never be treated as runtime
  // debug-history requests. If the LLM selected `get_debug_output`, rewrite
  // to the canonical scene inspection op.
  overrideSceneNodeListingPlanOps({ plan, userRequestText: userRequestText });
  preferMcpSceneCreationPlanning(plan, constrainedSupportedOperations);
  const enforcement = enforceSupportedOperations(plan, constrainedSupportedOperations);
  ...
  if (actionable && Array.isArray(enforcement?.rejected) && enforcement.rejected.length > 0) {
    return {
      ...
      details: {
        ...
        supported_operations: Array.isArray(constrainedSupportedOperations)
          ? constrainedSupportedOperations.filter((o) => o?.enabled).map((o) => o?.operation)
          : [],
      },
      raw_llm_response: rawText,
    };
