...
export async function runProjectConversationEdit({
  ...
  // Constrain edit-mode planning/execution to prototype operations only.
  // Edit mode is now a constrained operation interface (via RecipeEngine),
  // not a raw "MCP playground".
  const prototypeOpSet = new Set(Array.isArray(SUPPORTED_RECIPES) ? SUPPORTED_RECIPES : []);
  const discoveredEnabledMap = new Map(
  ...
  const constrainedSupportedOperations = Array.from(prototypeOpSet).map((op) => ({
    operation: op,
    // Local prototype ops that do not depend on GoPeak discovery must be enabled.
    enabled: op === "inspect_scene" || op === "create_script_file" ? true : discoveredEnabledMap.get(op) === true,
  }));

  supportedOperations = constrainedSupportedOperations;
