...
import { RecipeEngine, SUPPORTED_RECIPES } from "../RecipeEngine.js";
import { GOPEAK_DISCOVERY_DEBUG } from "../godot/GoPeakDebugFlags.js";
import { getOperationDefinition } from "../godot/GoPeakOperationRegistry.js";
...
function normalizeMcpActionName(action) {
  ...
  const aliases = {
    ...
    listSceneNodes: "list_scene_nodes",
    getNodeProperties: "get_node_properties",
    inspectScene: "inspect_scene",
    setNodeProperties: "set_node_properties",
    createScriptFile: "create_script_file",
  };
  return aliases[a] ?? a;
}
...
const PROTOTYPE_OPERATION_SET = new Set(Array.isArray(SUPPORTED_RECIPES) ? SUPPORTED_RECIPES : []);

function isAllowedPrototypeOperation(action) {
  const normalized = normalizeMcpActionName(action);
  return PROTOTYPE_OPERATION_SET.has(normalized);
}

async function applyMcpActions({ executor, plan, projectRoot }) {
  const mcpActions = Array.isArray(plan?.required_mcp_actions)
    ? plan.required_mcp_actions
    : Array.isArray(plan?.mcp_actions)
      ? plan.mcp_actions
  ...
  // Deep GoPeak/recipe debugging only when explicitly enabled.
  if (GOPEAK_DISCOVERY_DEBUG) {
    // eslint-disable-next-line no-console
    console.log("[ChangeExecutor][RecipeEngine] prototype op count", mcpActions.length);
  }

  for (const actionObj of mcpActions) {
    const rawAction = actionObj?.action ?? actionObj?.method ?? null;
    const action = normalizeMcpActionName(rawAction);
    const rawParams = actionObj?.params ?? {};
    const params = sanitizeCanonicalMcpParams(action, rawParams);
    const opCategory = safeString(getOperationDefinition(action)?.category).trim() || "unknown";

    if (!isAllowedPrototypeOperation(action)) {
      const msg = `Unsupported prototype operation: ${safeString(action)}. Edit mode only supports: ${Array.from(PROTOTYPE_OPERATION_SET).join(", ")}.`;
      errors.push({ action: safeString(rawAction ?? action), error: msg, unsupported: true });
      return { ok: false, errors, results };
    }

    const recipeRes = await recipeEngine.runRecipe({ operation: action, params, executor, projectRoot });
    results.push({ action, requested_action: action, result: recipeRes });
    // eslint-disable-next-line no-console
    console.log("[ChangeExecutor] operation outcome", {
      action,
      category: opCategory,
      ok: Boolean(recipeRes?.ok),
      phase: recipeRes?.phase ?? null,
    });

    if (!recipeRes?.ok) {
      errors.push({
        action,
        error: recipeRes?.error ?? "Recipe execution failed.",
        phase: recipeRes?.phase ?? null,
...
function planIndicatesProjectMutation(plan) {
  const affected = Array.isArray(plan?.affected_project_files) ? plan.affected_project_files : [];
  const fileChanges = Array.isArray(plan?.file_changes) ? plan.file_changes : [];
  const requiredMcp = Array.isArray(plan?.required_mcp_actions) ? plan.required_mcp_actions : [];
  const mcpActions = requiredMcp.length > 0
    ? requiredMcp
    : Array.isArray(plan?.mcp_actions)
      ? plan.mcp_actions
      : [];
  ...
  // Category-aware mutation gating:
  // read-only scene inspection operations should not trigger the disk mutation gate.
  const hasMutatingMcp = mcpActions.some((actionObj) => {
    const action = normalizeMcpActionName(actionObj?.action ?? actionObj?.method ?? null);
    if (!action) return false;
    const def = getOperationDefinition(action);
    const category = safeString(def?.category).trim().toLowerCase();
    if (!category) return true;
    return !category.includes("inspection");
  });

  // Success gating: only require disk mutation when the plan intends one.
  return affected.length > 0 || fileChanges.length > 0 || hasMutatingMcp || hasCli;
}
...
function isReadOnlyOperationCategory(category) {
  const c = safeString(category).trim().toLowerCase();
  return c === "scene_inspection" || c === "read_only";
}

function summarizeFinalActionCategory({ stageResults }) {
  const executed = Array.isArray(stageResults?.mcp?.results) ? stageResults.mcp.results : [];
  if (executed.length === 0) {
    return {
      action: "none",
      category: "none",
      hasKnownCategory: false,
      hasMutatingCategory: false,
      hasReadOnlyCategory: false,
    };
  }
  ...
  let hasMutatingCategory = false;
  let hasReadOnlyCategory = false;

  for (const entry of executed) {
    const action = normalizeMcpActionName(entry?.action ?? entry?.requested_action ?? null);
    if (action) actionNames.push(action);
    const category = safeString(getOperationDefinition(action)?.category).trim();
    if (category) {
      hasKnownCategory = true;
      categoryNames.push(category);
      if (isReadOnlyOperationCategory(category)) hasReadOnlyCategory = true;
      else hasMutatingCategory = true;
    }
  }
  ...
  return {
    action: uniqueActions.length === 1 ? uniqueActions[0] : uniqueActions.join(","),
    category: uniqueCategories.length === 1 ? uniqueCategories[0] : uniqueCategories.join(","),
    hasKnownCategory,
    hasMutatingCategory,
    hasReadOnlyCategory,
  };
...
function trimString(value, maxLen = 200) {
  const s = safeString(value);
  return s.length > maxLen ? `${s.slice(0, maxLen)}...` : s;
}

function toReadOnlyNodeSummary(nodes, maxItems = 50) {
  const list = Array.isArray(nodes) ? nodes : [];
  const trimmed = list.slice(0, maxItems).map((n, index) => {
    const node = isPlainObject(n) ? n : {};
    return {
      index: index + 1,
      name: trimString(node.name ?? node.node_name ?? ""),
      path: trimString(node.path ?? node.node_path ?? ""),
      type: trimString(node.type ?? node.node_type ?? ""),
    };
  });
  return {
    count: list.length,
...
function extractReadOnlyResultForAction({ action, recipeResult }) {
  const normalizedAction = normalizeMcpActionName(action);
  const output = isPlainObject(recipeResult?.output) ? recipeResult.output : {};
  const execution = isPlainObject(output?.execution) ? output.execution : {};
  const executionOutput = isPlainObject(execution?.output) ? execution.output : {};
  const params = isPlainObject(recipeResult?.inputs) ? recipeResult.inputs : {};
  const scenePath = safeString(params?.scene_path).trim() || safeString(params?.scenePath).trim() || null;

  if (normalizedAction === "list_scene_nodes") {
    const rawNodes = Array.isArray(executionOutput?.nodes) ? executionOutput.nodes : [];
    const nodes = toReadOnlyNodeSummary(rawNodes);
    return {
  ...
  if (normalizedAction === "get_node_properties") {
    const properties = isPlainObject(executionOutput?.properties) ? executionOutput.properties : {};
    return {
      action: normalizedAction,
      scene_path: scenePath,
      properties,
  ...
  if (normalizedAction === "inspect_scene") {
    const rawNodes = Array.isArray(output?.nodes) ? output.nodes : [];
    const nodes = toReadOnlyNodeSummary(rawNodes);
    const properties = isPlainObject(output?.properties) ? output.properties : null;
    return {
      action: normalizedAction,
      scene_path: scenePath,
      node_count: nodes.count,
...
function buildReadOnlyResultSummary(stageResults) {
  const executed = Array.isArray(stageResults?.mcp?.results) ? stageResults.mcp.results : [];
  const summaries = [];
  for (const entry of executed) {
    const action = normalizeMcpActionName(entry?.action ?? entry?.requested_action ?? null);
    const category = safeString(getOperationDefinition(action)?.category).trim();
    if (!isReadOnlyOperationCategory(category)) continue;
    const result = extractReadOnlyResultForAction({ action, recipeResult: entry?.result });
...
export async function executeChangePlan({
  ...
  // Prototype edit-mode constraint:
  // - No CLI actions.
  // - No raw file mutation improvisation for scene edits.
  // - Any mutable disk operations must be expressed as canonical prototype
  //   operations that run through RecipeEngine.
  const cliActions = Array.isArray(plan?.required_cli_actions)
    ? plan.required_cli_actions
    : Array.isArray(plan?.cli_actions)
      ? plan.cli_actions
  ...
  if (cliActions.length > 0) {
    return failWithStep({
      failed_step: "prototype_constraints",
      error: "Prototype edit mode does not support CLI actions. Rejecting this plan.",
      execution: stageResults,
      errors: [{ stage: "prototype_constraints", error: "cli_actions_not_supported", actions: cliActions.map((a) => a?.action ?? a) }],
      created_files: [],
      modified_files: [],
    });
  }
  ...
  const fileChanges = Array.isArray(plan?.file_changes) ? plan.file_changes : [];
  const nonScriptFileChanges = fileChanges.filter((fc) => fc && typeof fc === "object" && fc.type && fc.type !== "script_create");
  if (nonScriptFileChanges.length > 0) {
    const badTypes = [...new Set(nonScriptFileChanges.map((fc) => fc?.type).filter(Boolean))];
    return failWithStep({
      failed_step: "prototype_constraints",
      error:
        "Prototype edit mode only allows `file_changes.type=script_create` (mapped to `create_script_file`). Scene/file edits must be expressed as prototype operations.",
      execution: stageResults,
      errors: [{ stage: "prototype_constraints", error: "unsupported_file_changes_types", types: badTypes }],
      created_files: [],
      modified_files: [],
    });
  }

  // Convert `script_create` into deterministic `create_script_file` ops.
  const scriptCreateChanges = fileChanges.filter((fc) => fc && typeof fc === "object" && fc.type === "script_create");
  if (scriptCreateChanges.length > 0) {
    const toCreateScriptOp = (fc) => {
      const scriptPath = safeString(fc?.path).trim();
      const content = fc?.content;
      if (!scriptPath) throw new Error("script_create missing `path`.");
      if (typeof content !== "string" || content.trim().length === 0) throw new Error("script_create missing non-empty `content`.");
      return { action: "create_script_file", params: { script_path: scriptPath, content } };
    };
    try {
      const createdOps = scriptCreateChanges.map(toCreateScriptOp);
      const existingOps = Array.isArray(plan?.operations) ? plan.operations : [];
      const existingMcp = Array.isArray(plan?.mcp_actions) ? plan.mcp_actions : [];
      const existingReqMcp = Array.isArray(plan?.required_mcp_actions) ? plan.required_mcp_actions : [];
      plan.operations = [...existingOps, ...createdOps];
      plan.mcp_actions = [...existingMcp, ...createdOps];
      plan.required_mcp_actions = [...existingReqMcp, ...createdOps];
      plan.file_changes = [];
    } catch (err) {
      return failWithStep({
        failed_step: "prototype_constraints",
        error: "Invalid script_create file_changes; cannot map to create_script_file.",
        execution: stageResults,
        errors: [{ stage: "prototype_constraints", error: safeString(err?.message ?? err) }],
        created_files: [],
        modified_files: [],
      });
    }
  ...
  // Final safety: reject any planned prototype operations outside the
  // constrained RecipeEngine supported set.
  const plannedOps = Array.isArray(plan?.operations)
    ? plan.operations
    : Array.isArray(plan?.mcp_actions)
      ? plan.mcp_actions
      : [];
  const illegalOps = plannedOps
    .map((opObj) => normalizeMcpActionName(opObj?.action ?? opObj?.method ?? null))
    .filter((a) => a && !PROTOTYPE_OPERATION_SET.has(a));
  if (illegalOps.length > 0) {
    const unique = [...new Set(illegalOps)];
    return failWithStep({
      failed_step: "prototype_constraints",
      error: "Prototype edit mode rejected unsupported operations.",
      execution: stageResults,
      errors: [{ stage: "prototype_constraints", error: "unsupported_prototype_operations", operations: unique }],
      created_files: [],
      modified_files: [],
    });
  }
  ...
  stageResults.mcp = await applyMcpActions({ executor, plan, projectRoot });
  ...
  const finalActionCategory = summarizeFinalActionCategory({ stageResults });
  // Final success is category-aware:
  // - read-only actions (scene_inspection/read_only) succeed from execution+validation only.
  // - mutating actions must additionally pass the disk-mutation gate.
  const mutationGateEnabled = finalActionCategory.hasMutatingCategory
    ? true
    : finalActionCategory.hasReadOnlyCategory
      ? false
      : shouldGateOnProjectMutation;

  const mutationAchieved = mutationGateEnabled
  ...
  const validationOk = Boolean(stageResults.validation?.ok);
  const validationFailed = !validationOk;
  const mutationGateFailed = !mutationAchieved;
  const finalOk = validationOk && !mutationGateFailed;
  const decisionReason = validationFailed
    ? "validation_failed"
    : mutationGateFailed
      ? "mutation_required_but_not_detected"
      : mutationGateEnabled
        ? "mutation_verified"
        : "read_only_success";

  // eslint-disable-next-line no-console
  console.log("[ChangeExecutor] final success decision", {
    action: finalActionCategory.action,
    category: finalActionCategory.category,
    validation_ok: validationOk,
    mutation_gate_enabled: Boolean(mutationGateEnabled),
    mutation_achieved: Boolean(mutationAchieved),
    final_ok: finalOk,
  ...
  if (!finalOk) {
    const mutationErrors =
      mutationGateFailed && mutationGateEnabled
        ? [
            {
              mutation_gate: true,
              message:
  ...
  return {
    ...
    execution: stageResults,
    validation_result: stageResults.validation,
    // Result presentation is split by intent:
    // - mutation flows stay compact (status + validation)
    // - read-only inspection flows include concise returned data for humans.
    read_only_results: buildReadOnlyResultSummary(stageResults),
    created_files,
    modified_files,
    step_trace,
  };
