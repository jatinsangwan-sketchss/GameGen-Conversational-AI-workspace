...
function freezeDeep(value) {
  ...
  const OPERATION_DEFINITIONS = freezeDeep([
    ...
    },
    {
      ...
      prerequisites: ["project_required", "editor_bridge_required"],
      // Inspect-first rule:
      // `node_path` is resolved deterministically from scene inspection
      // (via list_scene_nodes + optional get_node_properties) using either:
      // - explicit node path, or
      // - explicit node name, or
      // - scene_root intent.
      //
      // This prevents brittle guessing (e.g. treating a node name as a node path)
      // and ensures property changes are applied to an actually-resolved target.
      required_params: ["scene_path", "properties"],
      optional_params: ["node_path", "node_name", "target", "target_intent", "property_changes"],
      // Raw editor-backed tools typically need the active connected project path.
      context_requirements: ["editor_bridge_context_required", "connected_project_path_context_required"],
    ...
    // -----------------------------
    {
      operation: "list_scene_nodes",
      category: "scene_inspection",
      prerequisites: ["project_required", "editor_bridge_required"],
      required_params: ["scene_path"],
      optional_params: [],
      context_requirements: ["editor_bridge_context_required", "connected_project_path_context_required"],
      // Inspect-first reads scene structure; there must not be any
      // fallback into runtime debug-history operations.
      placeholder_policy: "no_placeholders_required",
      fallback_policy: "fail_if_primary_unavailable",
      primary_execution_path: "mcp_tool",
      allowed_fallbacks: [],
    ...
    },
    {
      ...
      prerequisites: ["project_required", "editor_bridge_required"],
      required_params: ["scene_path"],
      optional_params: ["node_path", "node_name", "target", "target_intent"],
      context_requirements: ["editor_bridge_context_required", "connected_project_path_context_required"],
    ...
    },
    {
      ...
      optional_params: ["node_path", "node_name", "target", "target_intent"],
      context_requirements: ["editor_bridge_context_required", "connected_project_path_context_required"],
      // This operation is a composed read-only recipe in RecipeEngine:
      // it must call list_scene_nodes (and optionally get_node_properties),
      // not get_debug_output.
      placeholder_policy: "allow_root_node_default",
      fallback_policy: "fail_if_primary_unavailable",
